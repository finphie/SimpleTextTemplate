...
public sealed class TemplateReaderTryReadTemplateTest
{
    [Theory]
    [InlineData("{", "{", 1)]
    [InlineData("a", "a", 1)]
    ...
    [InlineData("z{{", "z", 1)]
    public void テンプレート文字列_文字列の範囲とtrueを返す(string input, string template, int consumed)
    {
        var utf8Input = Encoding.UTF8.GetBytes(input);
        var reader = new TemplateReader(utf8Input);

        reader.TryReadTemplate(out var range).Should().BeTrue();
    ...
    public void 空または識別子開始タグ_falseを返す(string input)
    {
        ...
        reader.TryReadTemplate(out var range).Should().BeFalse();
